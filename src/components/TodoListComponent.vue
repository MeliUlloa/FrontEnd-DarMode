...
// Definir variables reactivas para el tema y las tareas
...
<template>
    ...
    <div v-bind:class="['list-wrapper', { dark: modo.isDark }]" class=" max-w rounded overflow-y-auto shadow-lg mt-10 p-4 transition ease-linear">
        ...
        <div v-for="task in task.data" :key="task.id" class="wrapper relative group border-black my-2 transition ease-linear">
            <form v-on:submit.prevent>
                    <div class="absolute top-3 sm:top-4 left-5">

                    <!-- aca aplicar directiva @click para  cambiar estado de tarea -->
                    <div class="relative">
                        <input @click="taskStore.updateTaskStatus(task)"
                        type="ckeckbox"
                ...
                <input
                    ...
                    v-model="task.tarea"
                    type="text"
                     v-bind:class="['tarea', { dark: modo.isDark }]"
        ...
        <div class="px-6 py-2 mt-5">
            <span @click="task.showAll()" class="inline-block bg-gray-300 rounded px-3 py-1 text-sm font-semibold mr-2 mb-2 hover:bg-gray-500 cursor-pointer shadow-lg">Todas</span>
            <span @click="task.showCompleted()" class="inline-block bg-gray-300 rounded px-3 py-1 text-sm font-semibold mr-2 mb-2 hover:bg-gray-500 cursor-pointer shadow-lg">Completas</span>
            <span  @click="task.showPending()" class="inline-block bg-gray-300 rounded px-3 py-1 text-sm font-semibold mr-2 mb-2 hover:bg-gray-500 cursor-pointer shadow-lg">Pendientes</span>
